...
try {
    ...
    Invoke-HarnessTest 'manual review request READY and restored-result validators bind exact read-only evidence' {
        ...
        $manualRequest = [ordered]@{
            ...
            commit=('c'*40);productVersion='0.1.0-phase3b-dev.3';dllSha256=('d'*64);dllMvid='aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee';
    ...
    Invoke-HarnessTest 'schema-v2 final result recomputes subscenario totals from validated game evidence' {
        ...
        $v2Request=[pscustomobject]@{runId='recompute-test';scenario='fixture-intake';branch='codex/mounted-combat-feasibility';commit=('0'*40);productVersion='0.1.0-phase3b-dev.3';dllSha256=('a'*64);dllMvid=[Guid]::Empty.ToString();transactionToken=('b'*64);evidenceRoot=$recomputeEvidence;fixture=$fixture}
    ...
    Invoke-HarnessTest 'combat fallback preserves original launcher error without weakening strict evidence validation' {
        ...
        $fallbackRequest=[pscustomobject]@{runId='combat-fallback-test';scenario='combat-core-control-suite';branch='codex/mounted-combat-phase2-alpha';commit=('0'*40);productVersion='0.1.0-phase3b-dev.3';dllSha256=('a'*64);dllMvid=[Guid]::Empty.ToString();transactionToken=('b'*64);evidenceRoot=$fallbackEvidence;fixture=$fallbackFixture}
    ...
    $request = [ordered]@{
        ...
        productVersion = '0.1.0-phase3b-dev.3'; dllSha256 = ('ab' * 32)
    ...
    Invoke-HarnessTest 'horse companion registration validator binds exact production snapshots and lease restoration' {
        ...
        $registrationRequest = [pscustomobject]@{
            ...
            commit=('2'*40);productVersion='0.1.0-phase3b-dev.3';evidenceRoot=$registrationRoot
        }
        $initial = [ordered]@{
    ...
    Invoke-HarnessTest 'horse companion unmounted validator binds runtime behavior and exact cleanup' {
        ...
        $unmountedRequest = [pscustomobject]@{
            ...
            commit=('5'*40);productVersion='0.1.0-phase3b-dev.3';dllSha256=('6'*64)
        ...
        $unmountedObservations = [ordered]@{
            ...
            horseId='horse';horseBlueprintGuid='4016c7db400ab721ff125aef9e65e202';characterLevel=4;rank=4;upgradeRank=1
            expectedCharacterLevel=4;activationDefaultBuildContextPresent=$true;activationCharacterLevelAfterNativeTry=1
            deferredNativeAttempts=1;defaultBuildContextWaitFrames=0;lastDeferredDefaultBuildContextPresent=$false
            deferredCharacterLevelBefore=1;deferredCharacterLevelAfter=4;deferredProgressionSynchronized=$true
            runtimeSize='Large';speedFeet=50;hitPoints=40;armorClass=18;movementDisplacement=1.8
            movementRemainingDistance=0.1;ownerDisplacementDuringHorseMove=0.0
            fullAttackWeaponGuids=@($biteGuid,'b0e472a49ff2a294f93faa3ab757a4a5','b0e472a49ff2a294f93faa3ab757a4a5')
            realTimeAttackWeaponGuid=$biteGuid;realTimeAttackRules=1;realTimeAttackRolls=1;realTimeDamageRules=1;realTimeDamage=8
            turnBasedAttackWeaponGuid=$biteGuid;turnBasedAttackRules=1;turnBasedAttackRolls=1;turnBasedDamageRules=1;turnBasedDamage=7
        ...
        $unmountedArtifact = [ordered]@{
            schemaVersion=1;evidenceKind='horse-companion-unmounted';runId=$unmountedRequest.runId;scenario=$unmountedRequest.scenario
