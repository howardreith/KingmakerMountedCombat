...
try {
    ...
    Invoke-HarnessTest 'Phase 3E paired scheduler extends only one exact native command eligibility result' {
        ...
        Assert-Test ($phase3dHorseSource.Contains(
            ...
            $phase3dHorseSource.Contains('["schemaVersion"] = IsChunk4Core ? 22 : IsChunk4Play ? 21 : IsChunk4Charge ? 20 : IsPairedAllocation ? 17 : IsOrdinaryAttackControls ? 1 : IsPhase3hLoop ? (Phase3gTurnBased ? 9 : 10) : IsPhase3gControls ? 8 : IsPhase3fNativeControlScope ? 7 : 6,') -and
