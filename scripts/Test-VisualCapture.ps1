...
Assert-VisualCapture ($tranche.Contains('if (IsPhase3fNativeControlScope)') -and
    ...
    $tranche.Contains('IsOrdinaryAttackControls ? 1 : IsPhase3hLoop ? (Phase3gTurnBased ? 9 : 10) : IsPhase3gControls ? 8 : IsPhase3fNativeControlScope ? 7 : 6,')) 'C0 RT fixture bypasses all legacy shared-turn exercises before any mode transition'
