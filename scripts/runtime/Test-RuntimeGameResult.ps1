...
if ([string]$game.status -ceq 'PASS') {
    ...
    $expectedNativeWrites = if($game.scenario-ceq'persistence-p07-load'-and$request.persistenceCase-cin @('area-cross-entry-auto','area-cross-exit-auto')){1} elseif($campaignB){$campaignBWrites} elseif($game.scenario-ceq'persistence-p07-save'){if($request.persistenceCase-cin @('area-reload','area-cross-entry','area-cross-exit','serialization-cancel-output','prepare-removal')){2}elseif($request.persistenceCase-ceq'disable-during-load'){1}else{3}} elseif($alternating){if($game.scenario-ceq'persistence-p05-save'){2}else{1}} elseif ([string]$game.scenario -ceq 'persistence-p05-save') { 3 } elseif ([string]$game.scenario -cin @('persistence-p01-save','persistence-p02-save','persistence-p03-save','persistence-p04-save')) { 1 } else { 0 }
    $expectedWorkingLoads = if($absentKmc){0} elseif($game.scenario-ceq'persistence-p06-load'-and$request.persistenceCase-cin @('combat-missing','combat-ai')){4} elseif($game.scenario-ceq'persistence-p07-save'){if($request.persistenceCase-cin @('area-reload','area-cross-entry','area-cross-exit')){1}elseif($request.persistenceCase-ceq'cancel-wait'){4}elseif($request.persistenceCase-cin @('serialization-cancel','serialization-cancel-output','disable-reenable','prepare-removal')){1}elseif($campaignB){2}else{3}} elseif($game.scenario-ceq'persistence-p06-load'-and$request.persistenceCase-cin @('legacy','schema1','missing-rider','missing-mount','mismatched-profile','failed-area-load')){3} elseif($alternating-and$game.scenario-ceq'persistence-p05-load'){3} elseif ([string]$game.scenario -cin @('mounted-pair-load-safety','boundary-suite')) { 2 } else { 1 }
