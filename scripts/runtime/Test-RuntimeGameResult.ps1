...
if ([string]$game.status -ceq 'PASS') {
    ...
    $expectedWorkingLoads = if($game.scenario-ceq'persistence-p06-load'-and$request.persistenceCase-cin @('combat-missing','combat-ai')){4} elseif($game.scenario-ceq'persistence-p07-save'){if($request.persistenceCase-cin @('area-reload','area-cross-entry','area-cross-exit')){1}elseif($request.persistenceCase-ceq'cancel-wait'){4}elseif($request.persistenceCase-ceq'serialization-cancel'){1}else{3}} elseif($game.scenario-ceq'persistence-p06-load'-and$request.persistenceCase-cin @('legacy','schema1','missing-rider','missing-mount','mismatched-profile','failed-area-load')){3} elseif($alternating-and$game.scenario-ceq'persistence-p05-load'){3} elseif ([string]$game.scenario -cin @('mounted-pair-load-safety','boundary-suite')) { 2 } else { 1 }
