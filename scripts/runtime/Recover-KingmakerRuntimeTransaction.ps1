...
if(-not$PSCmdlet.ShouldProcess($liveMods,"recover exact KMC transaction $($raw.runId)")){if((Get-KmcDirectoryManifest $liveMods).digest-cne$before.digest){throw 'Recovery WhatIf purity failed.'};Write-Host 'Recovery WhatIf purity PASS; no external state changed.';return}
$lock=$null
