...
elseif ($schemaVersion -eq 2) {
    ...
    if($request.scenario-cin @('persistence-p05-save','persistence-p05-load')){
        ...
        if(-not$hasPersistenceCase-or$request.persistenceCase-cnotin @('step','conversion','round-effect','reaction','condition')){throw 'P03 requires its exact native commitment case.'}
