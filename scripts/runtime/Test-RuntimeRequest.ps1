...
elseif ($schemaVersion -eq 2) {
    ...
    if($request.scenario-cin @('persistence-p07-save','persistence-p07-load')){
        if(-not$hasPersistenceCase-or$request.persistenceCase-cnotin @('timeout','cancel-wait','locked-replace','serialization-cancel','serialization-cancel-output','disable-reenable','area-reload','area-cross-entry','area-cross-exit','area-cross-entry-auto','area-cross-exit-auto')){throw 'P07 requires its exact owned recovery case.'}
    }elseif($validation){
