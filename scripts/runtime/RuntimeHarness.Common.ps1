...
function New-KmcRuntimeResultV2 {
    ...
    if ($null -ne $ValidatedGameResult) {
        ...
        $fallbackName = if (@(Get-KmcSaveBackedRuntimeScenarios | Where-Object { $_ -ceq [string]$Request.scenario }).Count -eq 1 -and
            [string]$Request.scenario -notin @('fixture-intake','persistence-isolation','lifecycle-suite','combat-lifecycle-suite','chunk4-traversal-core','chunk4-traversal-slope','chunk4-area-cleanup','movement-suite','boundary-suite','presentation-suite')) {
            [string]$Request.scenario
