...
function Assert-KmcPhase3fStartingInstallation {
    ...
    # Phase 3G explicitly continues the user-installed preview.4, including the
    # exact cache generated by human play. Historical fallback pins remain valid
    # only for their original suites; each new suite pins its complete intake tree.
    if ((Get-KmcSha256 (Join-Path $KmcRoot 'Info.json')) -ceq 'e9a174ef03c8ad32639d9d82296189158fdcc9f8b9070269e46199ab77fd8a71') {
        $pins = @{
            'Info.json'='e9a174ef03c8ad32639d9d82296189158fdcc9f8b9070269e46199ab77fd8a71'
            'KingmakerMountedCombat.dll'='0dd6913ae2fb21b94d0681d355fc293747123508d32bebe9145aac153e975c07'
            'KingmakerMountedCombat.dll.20370.cache'='0dd6913ae2fb21b94d0681d355fc293747123508d32bebe9145aac153e975c07'
        }
    }
    ...
    # Chunk 4 preserves the actual owner-installed preview.37 and its human cache.
    # This exact registration grants no authority to restore an older intake.
    if ((Get-KmcSha256 (Join-Path $KmcRoot 'Info.json')) -ceq '0b9710862ae437570d7da779066bcb8a89b01279dcade9dd30b35d50709b20dd') {
        $pins = @{
            'Info.json'='0b9710862ae437570d7da779066bcb8a89b01279dcade9dd30b35d50709b20dd'
            'KingmakerMountedCombat.dll'='20080fdcf83c7628611c3f6354a3e47c3065e2b25998a9c01b39a69796ed57bb'
