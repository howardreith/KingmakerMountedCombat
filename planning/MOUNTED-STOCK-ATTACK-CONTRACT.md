...
Phase 3E final disposition (2026-09-05): `BLOCKED — CRITICAL` for ordinary unified-TB sequencing; Phase 3D RT and the Phase 3C separate-turn fallback remain `PASS`. Dev.12 proves one exact Horse Mount Primary TB action can retain Horse executor, natural weapon, Bite animation, target, rule, damage, and Standard ownership with zero rider cost. It later fires K9 at native turn completion, so that isolated success is not broadened into ordinary hostile-click unified-TB acceptance. Fallback version `0.1.0-phase3e-fallback.1` disables unified mode and the paired scheduler by default; it retains the qualified RT hostile-click melee implementation without claiming unified TB.
