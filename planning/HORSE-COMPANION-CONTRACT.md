...
Superseding status (2026-08-27T23:34:08Z): `BLOCKED — CRITICAL` at lifecycle qualification, while real Ranger creation and persistent ownership are technically PASS. Exact dev.23 run `20260827T230400Z-horse-levelup-dev23-passA` passed registration `13/0` and reached Horse `21/1`: four native Ranger commits created and retained the exact Ranger-4/rank-1 Horse, reciprocal AddPet/SetMaster ownership, active Large HorseRiding view, direct control, stock movement, Bite/Hoof/Hoof, and exact RT/TB Bite chains. The sole failure is now event-backed: direct damage changed the exact Horse from `0` to `27` damage against `11` HP, but no exact `IUnitLifeStateChanged` `Conscious -> Unconscious|Dead` transition occurred during the 30-second death window. Recovery and respec removal were not reached and remain unqualified. This blocks the lifecycle contract without proving that ordinary combat death itself is broken. Exact external restoration passed before evidence read; the failed artifact is not installed. No further lifecycle cycle, mounted testing, Paladin work, or merge is authorized without new user direction.

Superseding status (2026-08-27T21:43:31Z): IN PROGRESS - dev.22 run `20260827T205200Z-horse-levelup-dev22-passB` again proves real Ranger-4/rank-1 creation, persistent reciprocal ownership, active Large HorseRiding view, direct control, stock movement, and exact RT/TB Bite chains, but remains restored `FAIL 34/1` because its later `IsConscious` poll never observed a non-conscious frame. That polling gate is non-authoritative: exact `UnitLifeController` publishes `IUnitLifeStateChanged` inside its tick, and existing credited native-incapacitation evidence proves recovery may complete before the next scenario update. Dev.23 observes the exact Horse event by reference, requires one exact `Conscious -> Unconscious|Dead` transition, rejects a conscious poll alone, then retains visible recovery, feature removal, exact orphan destruction, Ranger append restoration, and isolation gates. Production companion behavior is unchanged. One final clean event-backed run and later human disk save/reload remain required.

Superseding status (2026-08-27T19:58:00Z): IN PROGRESS - dev.21 run `20260827T192000Z-horse-levelup-dev21-passB` technically proves the real Ranger level-up now creates and retains the exact companion: native Ranger-4/rank-1 selection and commit, reciprocal ownership, active Large HorseRiding view, direct control, stock movement, correct natural topology, and exact RT/TB Bite chains all pass. Its historical `FAIL 34/1` occurs only because the single 180-second scenario clock preempts `AwaitDeath` one frame after lethal damage is applied; no death state was evaluated. Dev.22 preserves the pre-lifecycle budget and adds a separate 30-second fail-closed death/recovery/respec window so the already-required cleanup assertions can execute. One clean final unmounted run and later human disk save/reload remain required.

Superseding status (2026-08-27T18:22:00Z): IN PROGRESS - dev.20 real Ranger qualification `20260827T173700Z-horse-levelup-dev20-passB` is immutable restored `FAIL 19/1`. Four native commits selected the exact Horse and created the exact pet at Ranger 4/rank 1, but it vanished before stable view spawn. Native `LevelUpController.Commit` turns the owner off/on after applying level-up; stock `AddPet` preserves its temporarily masterless pet for reattachment, whereas KMC's immediate orphan cleanup destroyed it during deactivation. Dev.21 defers cleanup one frame and destroys only an exact live KMC Horse that remains masterless and is no longer the former owner's reciprocal pet. This preserves native level-up reattachment while retaining exact respec/removal cleanup. Full unmounted runtime qualification and later human save/reload remain required.

Superseding status (2026-08-27T16:01:32Z): IN PROGRESS - dev.19 is immutable restored `FAIL 4/14` because its new dual-array hardening required the non-authoritative live `Features` field to equal the seven-entry Items surface; registration stopped before level-up. Installed `BlueprintFeatureSelection.Items` enumerates `AllFeatures`. Dev.20 therefore leaves `Features` reference/content untouched, leases only exact seven-entry `AllFeatures`, and retains canonical list+dictionary registration, `DlcType.None`, explicit Horse labeling, native AddPet, Ranger-4 rank-1/pet-level-2 semantics, and one eligible Items entry. Offline gates pass; one clean unmounted qualification and later human save/reload remain required.
...
- `BlueprintFeatureSelection.Items` filters `AllFeatures` through `IsDlcAvailable()`. The live `Features` field is a distinct non-authoritative surface and is left reference/content untouched. KMC owns one compare-before-restore append lease on exact seven-entry `AllFeatures`, while KMC-owned features use base-game `DlcType.None`.
