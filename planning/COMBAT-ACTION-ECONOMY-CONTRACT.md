...
| Diagnostic combat entry | `UnitMemoryController.AddToMemory` token `0x0600916F` is buffered, rejects a peaceful target faction before enqueue, and later skips a hidden queued target. Probe O proves its reverse request can remain absent. `UnitEntityData.Memory` `0x060082F8` is exact group memory; public `UnitGroupMemory.Add/Remove/Contains` are `0x06001F2C/2D/31`. Stock prepare/cooldown/leave ticks remain `0x0600936F/34A/368`. | Controlled target provisioning captures the exact rider and transient-target groups, leases exactly one memory entry in each, refreshes only their detect timestamps while awaiting dispatch/outcome, and removes both before target/group destruction. Qualification still waits for both entries, stock combat membership/preparation, awake rider, Default mode, positive time, and exact dispatch readiness. KMC never edits faction, stealth, combat membership, initiative, cooldown, or a save. |
