...
Phase 2A presentation runtime qualification remains `IN PROGRESS`. The seven exact rows are `pose-idle`, `pose-walk-run`, `pose-turn-stop`, `pose-doorway-formation`, `pose-equipment-variants`, `ui-selection-portrait-actionbar`, and `camera-follow-and-command-routing`; all seven are now technically qualified twice in fresh processes. Two complete-suite processes, the exact manual-review build/launcher, and subjective acceptance remain required.
