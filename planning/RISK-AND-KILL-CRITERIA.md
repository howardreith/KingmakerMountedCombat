...
No architecture kill criterion fired. Architecture B's frozen F0 runnable pair-only movement, lifecycle, cleanup, boundary, native-asset, and external-restoration evidence is positive. The controlling stop is a current P0 safety hold: F1 is descriptor-requalified but cannot be admitted for runtime while two valued non-KMC saves have unauthorized cross-epoch deltas. The three mandatory Proceed rows remain downstream and unexecuted.

Frozen F0 gate ledger: source 21 PASS / 0 FAIL; Release build PASS; pure/component 112 PASS / 0 FAIL; visual-capture contracts 12 PASS / 0 FAIL; guarded harness/protocol 105 PASS / 0 FAIL; assembly-backed 69 PASS / 0 FAIL (Kingmaker 58, Wrath 11). The F0 named runtime-row disposition is 22 PASS / 0 attributable FAIL / 3 DEFER — EVIDENCED. Historical runner, validator, capture, and instrumentation failures remain preserved but are not architecture kills. F1 has descriptor-requalification evidence, no runtime admission, and no runtime row result.

Safety-guard implementation commit `29e594d6d053824a47ac22b9dd702aede1036031` is on `codex/mounted-combat-feasibility`. Its exact four-file scope is `scripts/runtime/RuntimeHarness.Common.ps1`, `scripts/runtime/Test-KmcFixtureGuard.ps1`, `scripts/runtime/Invoke-KingmakerRuntimeScenario.ps1`, and `scripts/Test-Harness.ps1`, with SHA-256 `6605ef74d10a7a99ea1ec21550689a2fa3600ecde89354389acd5593bfa525ca`, `79129c6041fe55403b35750b563fa32d4f90c83e6f93f956053331af19e48647`, `3259ff4d6a41505d9faea3655cb79a87ee487b8e69f8ef72ab2505153a69bb81`, and `f44133f5f6e88ac2b028e609b0dbb69d03161b598e40fbf75b29015872e4563e`. AST `4/4`, source `21/0`, Release build, component `112/0`, visual `12/0`, harness `132/0`, assembly-backed `69/0`, and diff check passed; two independent read-only reviews returned GO for the guard, not for runtime admission.
...
Frozen F0 inventory was digest `58a54974423316041e29d32b1093c5d6775dbd67fe2b37ffaea3c9ae27de19be`, `271` files, `3178063612` bytes. Current F1 differs in exactly three entries and no others: authorized Working `718821` / `639222573406912936` to `816452` / `639222975870964407`; unauthorized `Auto_1120` `204829` / `639222474628040540` to `333208` / `639222975467301685`; unauthorized `Quick_438` `625411` / `639220694761623881` to `809565` / `639222975512345112`. The transaction-level equality does not establish cross-epoch continuity. The project must not restore or mutate the two valued saves.

The hardened final real `AuditWorkingContinuity -WhatIf` pinned current qualification SHA-256 `95e9a3c6203f11cfdd7e615849eb61ebc060131075f6a7f40771dcd7eeb68a7a` and frozen F0 save-transaction authority SHA-256 `b25f80c799207657650cf29118078edc4685bcbb51e1609ba8b167cab13052e0`. It produced the expected safety FAIL, `Save write allowlist violation: Auto_1120.zks, Quick_438.zks`, without changing either pinned record and without leaving a process, lock, sentinel, or live KMC tree. No runtime scenario executed; the rejection confirms the active hold rather than firing an architecture kill criterion.
...
| K3 | Pair fails valid turns/doorways passed by unmounted control | F0 turns/corners passed A/B. Its unmounted Mammoth passed the doorway control and the mounted route was accepted and stable; native hostiles then triggered correct `CombatStarted` cleanup before a corrected repeat completed. This is F0 fixture contamination, not an attributable traversal failure. F1 is blocked from runtime with no doorway result yet | DEFER — EVIDENCED | After safety admission, exact same-Mammoth unmounted control and mounted traversal in two fresh F1 processes |
| K4 | Selection/formation cannot restore | F0 pair selection retention and every cleanup restoration passed, but away/back and group formation could not execute because F0 had no eligible directly controllable non-pair; no restoration failure occurred. F1 is blocked from runtime with no selection/formation result yet | DEFER — EVIDENCED | After safety admission, run selection and meaningful stock group formation A/B against F1 Working |
...
| K12 | External state cannot be restored exactly | Every F0 live transaction restored exact live Mods and save metadata. F1 requalification preserved its then-current inventory, kept Baseline byte-identical, committed its state, and left no process or transaction residue. The hardened pinned audit correctly rejects unauthorized `Auto_1120.zks` and `Quick_438.zks` cross-epoch drift; no K12 failure is assigned, but runtime admission is unsafe | BLOCKED — CRITICAL | User-owned resolution/authority for `Auto_1120` and `Quick_438`; project must not restore or mutate them |
...
## Active critical completion hold

Status: BLOCKED — CRITICAL

Fixture descriptor identity is not a blocker. The F1 exact audit is Baseline=1 / Working=1 / KMC-looking near-match=0, with distinct paths, exact internal names, shared `GameId`/`GameName`/`Area` identity, Baseline immutability, and Working-only write authorization. External-state admission is blocked by the two unauthorized cross-epoch valued-save deltas.
...
All three remain F0 `DEFER — EVIDENCED`. They are not K3/K4 failures and therefore do not authorize the Pivot path, but mission §§18 and 27.7 do not allow them to be converted into Proceed passes. F1 descriptor requalification does not reclassify them, because F1 is not runtime-admitted and no F1 scenario has executed. The visual classification also does not substitute for mechanical qualification. Architecture B remains the provisional product-fit leader and stays default-off; Phase 2 is not authorized.

The revised `KMC_AUTOMATION_WORKING` descriptor is qualified and Baseline remains immutable, but F1 is not admitted for runtime. Exact next action: HOLD without package, runtime-scenario `-WhatIf`, publication, or game launch until the user resolves or explicitly authorizes the changed valued saves `Auto_1120` and `Quick_438`. The project must not restore or mutate them. Only then may the pinned continuity audit and admission decision be repeated. Phase 2 remains outside authorization.
