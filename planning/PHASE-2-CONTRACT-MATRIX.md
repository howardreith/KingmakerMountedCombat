...
| Persistence/uninstall | Direct pre-save cleanup only; real load prefix; no save round trip | Clean dismount before save/load/area; no saved fact, unit part, or relationship; disabled/uninstalled mod leaves ordinary units | Exact native delivery, save-safe telemetry, load with feature disabled where safe | IN PROGRESS — transient design implemented; runtime unrun |
| Player action | Diagnostic-only activation in Phase 1 | Owned `HideAndDontSave` overlay delegates to a deterministic evaluator/controller; `Mount`/`Dismount`, exact multi-reason feedback, rider selection normalization, fail-closed cleanup | Component `7/0`; harness support; two fresh-process flows still required | IN PROGRESS |
| Native lifecycle | Several Phase 1 handlers were invoked directly | Exact global handlers plus save/load Harmony prefixes write a bounded ordered delivery ledger with relationship before/after and cleanup result | Component `3/0`; exact handler signatures mapped; native runtime rows still required | IN PROGRESS |
