...
| Player action | Diagnostic-only activation in Phase 1 | Owned `HideAndDontSave` overlay delegates to a deterministic evaluator/controller; `Mount`/`Dismount`, exact multi-reason feedback, rider selection normalization, fail-closed cleanup | Controller runtime availability `29/0` twice; mount/dismount `44/0` twice; actual IMGUI repaint/button telemetry included in the new UI row | IN PROGRESS — controller path qualified; actual repaint row unrun |
| Native lifecycle | Several Phase 1 handlers were invoked directly | Exact global handlers plus save/load Harmony prefixes write a bounded ordered delivery ledger with relationship before/after and cleanup result. Mode probe changes only `SettingsEntityBool.m_Cached`, invokes the registered callback, then restores cache and persisted-value identity. | Component `128/0`; harness `138/0`; exact Kingmaker contracts `64/0`; save `63/0`, area `47/0`, mode `60/0`, and disable/residue `63/0`, each twice | PASS — four bounded native rows repeatably qualified |
...
| Packaging/publication | Frozen Phase 1 diagnostic only | Phase 2A presentation-review identity `0.1.0-phase2a-dev.2`; exact Kingmaker AnimationModule allowlisted; guarded non-force branch publication; no release/main merge | Clean package allowlist, hashes/MVID, local/remote equality | IN PROGRESS |
