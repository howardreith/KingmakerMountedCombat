...
Immutable dev.7 run `20260904T195400Z-phase3e-dev7-horse-tb-gate2` is also outer/game `FAIL 42/2` at `AwaitCombatMount`, after an independent audit-before-read passed exact suite/save/Mods/Baseline/Working restoration. The dev.7 rider and Horse AI/readiness correction passed: both exact leases were active and stable before target creation, both containers remained empty, rider hands/equipment became idle, and the native Mount click admitted one exact player-created rider Move-slot `UnitUseAbility` at legal adjacency with `CanStart=true`, no cooldown, and no approach. It remained unstarted for 30 seconds and ran only after cleanup returned to RT. This confirms a remaining native turn-start gate, not a paired scheduler failure; no Gate 2 row, repair cycle, or kill criterion is credited.
