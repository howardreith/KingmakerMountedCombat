...
- the exact shell is bound by reference, player origin, rider executor, Horse target, Move slot, no queue entry, admission/start/terminal frame, eligibility true/false counts, duplicate-frame count, UI guard, current turn/status, and all `ShouldStartCommand` inputs;
...
Dev.5 through dev.7 live Horse TB processes remain immutable uncredited failures at distinct pre-scheduler diagnostic boundaries, each with exact independent restoration. Dev.8 is offline-complete and corrects only the artificial rider-turn intake while adding one comprehensive native lifecycle trace. Next: coherent guarded publication, clean package/suite/WhatIf, and one fresh existing Horse TB tranche with independent audit-before-read. Attribute the existing pair-aware `ContinueActing`, mount-ledger preparation, exact movement adapter, hostile-click sequencing, combat Mount/Dismount semantics, and five-foot-step rows before adding missing instrumentation. `exact-turn-completion` remains TODO until explicit native next-combatant advancement is observed.
