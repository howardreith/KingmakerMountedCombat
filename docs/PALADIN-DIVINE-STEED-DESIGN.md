...
Dependency update (2026-09-04T19:05:03-04:00): Phase 3E has user authorization for an original pair-local command scheduler and has qualified only its isolated mount-primary Gate 1 twice (`140/0`). Broader sequencing and lifecycle gates remain `IN PROGRESS`; the separate-turn fallback remains available. Dev.8/dev.9 are diagnostic-only combat-Mount intake work and add no Paladin authority, blueprint, feature, spell, progression, companion, or implementation. Paladin Divine Steed remains design-only and unauthorized.

Dependency update (2026-09-04T03:05:00-04:00): Phase 3D is `BLOCKED — CRITICAL`. Exact Kingmaker can admit but cannot advance a mount-owned non-AoO TB command while the rider remains the real current-turn principal; Wrath supplies paired command links and a two-actor turn controller that Kingmaker lacks. A future Divine Steed design must not depend on the unqualified Phase 3D coordinator as though shared-turn acceptance occurred. It remains design-only until a separate mission chooses the accepted separate-turn fallback, authorizes a purpose-built paired-command scheduler, or revises independent ownership. No Paladin implementation was started.

Historical dependency update (2026-08-30T22:31:43Z; superseded by the 2026-09-04 disposition above): Phase 3D implements a rider-led shared initiative/turn model with rider portrait/selection principal, separate rider and mount action ledgers, combat Mount/Dismount reconciliation, ordinary stock attack intent, and native ranged routing. At that checkpoint these were offline-green and awaiting fresh guarded runtime plus human acceptance. The later runtime blocker means a future Divine Steed must not treat that coordinator as accepted. This remains a design dependency only: Paladin implementation is explicitly unauthorized.
