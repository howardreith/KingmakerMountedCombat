...
Current development version is `0.1.0-phase3d-dev.4`. The immutable dev.1 package failed before gameplay at nested registration admission. Dev.2 repaired that boundary and exposed the Horse presentation precondition. Dev.3 repaired the Horse coordinate boundary, then its first RT process admitted the exact native Rider Primary cast request while still on the first combat-entry frame and never reached ability delivery. Dev.4 is the single bounded native-shell repair: the diagnostic waits for Kingmaker's complete RT command-start gates, and only exact KMC mounted primary intent shells delegate their pre-dispatch LoS/approach ownership to the existing mount-owned child transaction.
