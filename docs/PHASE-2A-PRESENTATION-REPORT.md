...
The first native-area process is preserved as `FAIL` `48/3`: `Game.ReloadArea` returned before native unload delivery, so the producer captured its latch prematurely even though the exact successful unload and all later area stages arrived asynchronously. The first repaired-package process passed in-game `47/0`, including exact native delivery and fresh-world cleanup, but external validation rejected same-frame loading-start evidence because Unity had not yet flushed deferred destruction of the owned anchor/component. The state machine now requires a later Update observation after cleanup; strict validators remain unchanged. The area row remains unqualified pending a new clean packaged A/B rerun. Mode, disable/uninstall, pose, actual UI, and camera rows also remain open. No visual or uninstall PASS is claimed.
