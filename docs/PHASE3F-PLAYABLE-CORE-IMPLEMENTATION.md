...
Installation update, 2026-09-05T23:00:13Z: the user explicitly requested deployment after this mission's test closure. **Exact preview.4 is now intentionally installed in local UMM**, with guarded fallback/cache backup and independent verification of the candidate, unchanged foreign Mods and saves. No game launched or qualification changed. The [current resume record](../AUTONOMOUS-RESUME.md) contains operation/backup/audit identities. Restoration statements below describe the earlier temporary test transactions; do not restore fallback as routine cleanup now.
