...
Bootstrap intake: full actual WhatIf purity PASS on source `8612544`. Run `20260920-chunk5-isolation-A` failed before launch on profile inventory ordering; 379 files/64 directories have identical bytes. Corrected ordinal profile digest has a regression check (profile protection 4/0). No game process, staging or human-data mutation occurred. Next: repeat the guarded isolation bootstrap, then implement native P01; the extensionless archive member avoids the installed loader's treatment of unknown `.json` members as area files.
