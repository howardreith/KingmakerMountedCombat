...
Preview26/source6585309 adds an unqualified P04 RT spent-action control: native current debt/reference restoration without a TB turn and preparation/cooldown gating until rebind. Preview27 repairs a demonstrated nested-iteration side effect in the projectile guard: exact native HashSet/List reads replace the public iterator, preserving an already-active native iteration. Arrival remains separate from completed OnHit effects. Installed assembly/storage63/0, source22/0 and components423/0 pass; unchanged harness258/0, metadata44/0 and owned fixtures77/0 remain. No native26 run occurred. Preview27 unmounted-save-A3/1 failed before saving because the diagnostic Default-mode gate blocked its native auto-pause handler; the trace shows Pause with initiative still pending. Preview28 permits Pause through that diagnostic gate, preserving production27. Human intake restored03:25:17.1331823Z. Next unmounted source/cold-B, then mounted control. Active-command deferral remains TODO; no RT gameplay PASS is claimed.
