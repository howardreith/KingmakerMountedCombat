...
Preview26 adds an unqualified P04 RT spent-action control: serialize native current debt and references, restore without a TB roster, and block preparation/cooldown ticks until semantic rebind. Projectile arrival is separate from OnHit completion; a weak completion marker and full native/pending-list observation guard that barrier. Native active-command deferral is still TODO. Offline checks: source22/0, components423/0, harness258/0, metadata44/0, installed assembly/storage61/0, owned fixtures77/0. Next actual unmounted source/cold, then mounted control; no RT gameplay PASS is claimed.
