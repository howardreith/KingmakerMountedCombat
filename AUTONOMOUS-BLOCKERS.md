...
The combat protocol checkpoint is guarded-published at `974ca6f2613e4e92a061fdecc7d73b8f3f36cd0c`. First guarded run `20260816T033400Z-rider-hit-rt-probeA` failed before combat because the transient target did not pass every strict safety predicate; a separate final-result allowlist omission also rejected the new combat artifact after exact restoration. Neither is a critical mission blocker and the run receives no qualification credit. The narrow offline-green repair preserves all gates, adds exact false-gate diagnostics, and adds end-to-end final-result coverage. Commit and guarded-publish it, package clean HEAD, pass full-pin WhatIf, and run one fresh diagnostic process. Do not relax a provisioning gate without exact runtime evidence. Do not launch Wrath, touch another project, create a public release, merge `main`, or implement any unauthorized feature.
