...
Stationary mounted rider melee hit is qualified twice in both real-time and turn-based modes. No critical blocker is active. Two deterministic miss attempts are preserved uncredited `FAIL` `46/1`: every mechanic and restoration assertion passes, but their exact native result categories differ (`ArmorAC`, then `Miss`). Installed `ModifiableValueArmorClass.SelectMissReason` token `0x06007F2D` proves this is an intentional random choice among exactly `Miss`, `DodgeAC`, `ArmorAC`, and `ShieldAC`; authoritative `RuleAttackRoll.IsHit=false` and zero damage agree. The bounded codomain repair and complete offline gates pass. Exact next action is commit/publication/package/WhatIf and a new fresh same-package miss A/B sequence with independent audits. Invalid-target, target-death, cleanup, and non-mounted-control remain open. Do not infer qualification, weaken gates, launch Wrath, touch another project, create a public release, or merge `main`.
