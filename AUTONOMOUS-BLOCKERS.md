...
Probe D `20260816T044500Z-rider-hit-rt-probeD` from guarded-published `a7a1773ca81e3c0eb6c64148cfcabe8f101c5c8f` truthfully fails before target creation because the accepted active Mammoth has no additional-limb slot; it receives no qualification credit. Exact local contracts reject the provisioning theory: stock natural attacks use `Body.EmptyHandWeapon`, and native single-attack order is eligible primary hand, eligible secondary hand, then additional limb. The narrower zero-mutation repair mirrors that exact order, requires the stock Mammoth empty-hand blueprint through `PrimaryHand`, and proves identical before/after limb counts. Finish the complete gate, commit and guarded-publish, package clean HEAD, pass full-pin WhatIf, then run one fresh stationary real-time probe. Do not relax a gate, launch Wrath, touch another project, create a public release, merge `main`, or implement an unauthorized feature.
