...
Probe C `20260816T041500Z-rider-hit-rt-probeC` from guarded-published `8ad8c1d5fcb874e4e249b2361cef18abc78cce25` passes group hostility, raw AI, no-loot, and exact three-part cleanup, but truthfully fails before combat because a fresh stock companion template has no initialized natural-weapon limb. This is not a critical blocker and receives no qualification credit. The narrow repair uses exact native `UnitBody.AddAdditionalLimb` only after deriving and validating the accepted active Mammoth's first natural-melee blueprint; durable evidence must prove matching source/target weapon IDs, initial absence, non-loot, zero XP, no AI, hostility, unique group, and zero cleanup residue. Finish gates, commit and guarded-publish, package clean HEAD, pass full-pin WhatIf, then run one fresh stationary real-time probe. Do not relax a gate, launch Wrath, touch another project, create a public release, merge `main`, or implement an unauthorized feature.
