...
Probe G `20260816T053000Z-rider-hit-rt-probeG` from guarded-published `16d48313cfd313ef13b67d3f7a743f1c6756b249` passes exact target creation and near-boundary placement but truthfully fails when the native/Harmony click returns false; no command, rule, damage, action cost, or movement occurs, and it receives no qualification credit. The old row did not preserve whether the prefix declined or the controller rejected context, then obscured the rejection with an `AwaitOutcome` deadline. The narrow repair provisions visibility only on the exact transient target, evaluates exact fog/view/player visibility/click-object/attackability/melee gates, and immediately cleans up on rejection while preserving controller feedback and armed/active state. Complete offline gates pass (`21/0`, `183/0`, `17/0`, `152/0`, assembly `153/0`). Commit and guarded-publish, package clean HEAD, pass full-pin WhatIf, then run fresh Probe H. Do not alter area/global fog, relax a product gate, launch Wrath, touch another project, create a public release, merge `main`, or implement an unauthorized feature.
