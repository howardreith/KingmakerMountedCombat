...
Stationary mounted rider melee hit is qualified twice in both real-time and turn-based modes. No critical blocker is active. Miss run `20260816T121500Z-rider-miss-rt-passA` passes `47/0`; same-package run `20260816T123000Z-rider-miss-rt-passB` is preserved uncredited `FAIL` `20/1` because the transient target fell out of `AwakeUnits` before click. Exact installed evidence proves default `Wake()` only extends `AwakeTimer`, while `SleepingUnitsController` rebuilds the awake set and honors per-unit `Sleepless`. A target-only false-to-true lease with exact restoration, schema-v8/v9 evidence, legacy-schema compatibility, and complete offline gates now passes. Exact next action is commit/publication/package/WhatIf and two new fresh same-package miss A/B processes with independent audits. Invalid-target, target-death, cleanup, and non-mounted-control remain open. Do not infer qualification, weaken gates, launch Wrath, touch another project, create a public release, or merge `main`.
