...
Stationary mounted rider melee in real time remains qualified twice. No critical blocker is active. TB probes A-D are immutable uncredited failures. Probe D reaches and damages the exact target through one rider-owned native child under the exact acting rider turn, with correct resource ownership and zero duplicates/repaths, but that child later ends `Interrupt`; the wrapper therefore correctly fails. A bounded terminal-lifecycle trace is locally green and preserves the strict `Success` assertion. Exact next action is commit/publication/package/WhatIf and fresh Probe E to identify the exact interrupt caller before any behavior repair. Do not infer TB, miss, invalid-target, target-death, non-mounted, or Mammoth-attack qualification; do not weaken gates, launch Wrath, touch another project, create a public release, or merge `main`.
