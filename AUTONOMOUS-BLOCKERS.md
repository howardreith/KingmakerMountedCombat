...
Stationary mounted rider melee in real time remains qualified twice. No critical blocker is active. First TB run `20260816T093000Z-rider-hit-tb-passA` is uncredited `FAIL` `10/1`: exact installed `IsInTurnBasedCombat` requires combat and therefore cannot be the pre-combat enable gate. The repaired row waits for the exact temporary setting before mount, retains the full predicate after native combat entry, and preserves early schema-v5 failures without relaxing PASS. All offline gates and independent restoration pass. Exact next action is clean commit/publication/package/WhatIf and a fresh TB probe. Do not infer TB, miss, invalid-target, target-death, non-mounted, or Mammoth-attack qualification; do not weaken gates, launch Wrath, touch another project, create a public release, or merge `main`.
