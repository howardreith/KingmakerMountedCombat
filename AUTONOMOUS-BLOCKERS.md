...
Probe O `20260816T081000Z-rider-hit-rt-probeO` from guarded-published `d8a6224c1f3be3169ce3bbc09e9927f106695ca6` truthfully fails `20/1` before click because the buffered native memory controller never materializes the target-group-to-rider half of the requested pair. There is no qualification credit and no combat-mechanics finding. Exact Kingmaker code exposes unobservable faction/stealth admission gates in that buffer. The narrow diagnostic-only worktree instead leases exactly two public group-memory entries, refreshes only their detect timestamps while owned, validates them before dispatch/completion, and removes both before destroying the transient group. It never edits a faction, stealth, initiative, cooldown, combat membership, production attack path, or save. Complete offline gates pass (`21/0`, `192/0`, `17/0`, `155/0`, assembly `176/0`). Commit and guarded-publish, package clean HEAD, pass full-pin WhatIf, then run fresh Probe P and independently audit restoration before evidence inspection. Do not weaken product/native gates, launch Wrath, touch another project, create a public release, or merge `main`.
