# Active Chunk 5 — cross-area continuation blocked by destination terrain, 2026-09-23 UTC

IN PROGRESS; no product defect is proven and no hard stop exists. The P07 same-area retained-view gate is qualified source/cold on preview58. The two cross-area cases are implemented and every one of their persistence assertions passes natively, but the shared ordinary-movement continuation cannot complete at the DungeonStartHub arrival point, which is a located fixture-terrain blocker of the same class as the historical BD/BF physical-location failures.

Three preview59-62 cross-entry roots record it. Preview59 `20260923-chunk5-P07-area-cross-entry-save-A` FAIL0/1 was a declaration defect, not terrain: SaveManager.SaveRoutine06008029 is an iterator, so KMC's save-admission prefix runs while LoadArea06000CD5 synchronously constructs the enumerator in the departure area, and the authored after-entry autosave therefore spans two areas across its admission and commit boundaries. Preview60 corrected that and reached FAIL25/1 at normal-native-movement; preview61 and preview62 reproduce it identically.

The measured boundary is deterministic. From origin (-25.8238926, 11.7294607, 25.18078) the mount reaches (-25.3390751, 12.929203, 27.08651): 1.97m of horizontal progress with a 1.20m climb, then an empty A* path and native result Interrupt while still 1.19m from an approach point the trace placed at the origin's elevation. Party movement had settled to zero, stuckTime was 0, no navmesh obstacle or static block was reported, and all eight full-footprint probes at the mount's own corpulence 1.06 radius returned residual 0.0. ObstacleAnalyzer.TraceAlongNavmesh and the real footprint-aware route therefore disagree at this exact arrival point. Crowding and footprint occupancy are both disproved; do not replay this route or relax the Success and better-than-one-metre progress checks, and never accept Interrupt as success.

Next action requires a fixture decision, not another replay of the same route. Either qualify the ordinary continuation after an authored return transfer into the already qualified Area_Dwarf_1 terrain, which additionally needs that area's own BlueprintAreaEnterPoint GUID and raises the expected suspension/resume counts to two, or locate a different hub arrival point or destination whose native path a Large mount can actually complete. The existing cross-area persistence evidence through the post-transfer write is unaffected either way and must not be re-derived.

Actual human chunk4-preview54 DLL2203a68ca13dfebd1fc52be7c15521f3c2503c98cd53a891dd210ba0611019e9 and current data/settings/caches/foreign Mods restored 2026-09-23T01:29:03.4178698Z; no game process or transaction lock, both automation fixtures byte-identical, BASELINE immutable. [Single report](docs/CHUNK5-PERSISTENCE.md) and [resume pointer](AUTONOMOUS-RESUME.md) retain the exact next action and the mandatory remaining gates. The approved DLL-only5 MiB limit remains the sole capacity exception.
---
