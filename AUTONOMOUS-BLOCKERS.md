...
Stationary mounted rider melee in real time remains qualified twice. No critical blocker is active. TB probes A-E are immutable uncredited failures. Probe E again reaches and damages the exact target through one rider-owned native child under the exact acting rider turn, with correct resource ownership and zero duplicates/repaths, but that child later ends `Interrupt`; the wrapper therefore correctly fails. Its first-frame caller is only `UnitCommand.Interrupt`, which is insufficient to identify the external source. A bounded 12-frame caller-chain diagnostic is locally build-green and preserves the strict `Success` assertion. Exact next action is full gate, commit/publication/package/WhatIf, and fresh Probe F to identify the external caller before any behavior repair. Do not infer TB, miss, invalid-target, target-death, non-mounted, or Mammoth-attack qualification; do not weaken gates, launch Wrath, touch another project, create a public release, or merge `main`.
