...
Stationary mounted rider melee hit is qualified twice in both real-time and turn-based modes. No critical blocker is active. Tranche C is not complete: invalid-target, target-death, cleanup, and non-mounted-control remain open, while the isolated deterministic miss row is offline-green but not yet runtime-qualified. Exact next action is commit/publication/package/WhatIf and fresh same-package miss A/B with independent audits. Do not infer miss or later controls before their evidence, or Mammoth-attack qualification; do not weaken gates, launch Wrath, touch another project, create a public release, or merge `main`.
