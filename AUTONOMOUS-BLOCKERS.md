...
IN PROGRESS. No critical hard stop is proven. Isolation-C native/outer PASS, with exact human intake restored. P01-save-D now PASS23/0 native plus outer restoration. Fresh P01-load-A failed early actor binding because native GameId is assigned after PostLoad; preview.7 fixes that boundary with408/0 components and23/0 archive contracts. Native cold retry and P02-P08 remain IN PROGRESS/TODO. Preview.7 packaging FAIL at the existing 4 MiB DLL-entry limit (4,197,376 bytes). The owner approved the narrow 5 MiB DLL-only exception; the unchanged other bounds and new size regressions must pass before qualification under a new package identifier. No native preview.7 run or critical native hard stop is claimed. Source archive and actual human intake were preserved/restored. No critical native invariant is proven to block progress. See [the single report](docs/CHUNK5-PERSISTENCE.md). Human preview.54 is the current intake, not historical preview.37/preview.13. Isolation-B failed before fixture load; its owned process exited normally and exact human installation/data/settings were restored. No game or lock remains. Normal sandbox setup fails; individually reviewed bounded commands work.
