...
## Cleared blocker: fixture load context
...
Exact tracing found the first implementation defect: `WorkingFixtureLoader` called `Game.LoadGame` directly from the title screen. The native PC path is `MainMenu.LoadGame` -> `EnterGame` -> `LoadBaseMechanics` -> `Game.Initialize` -> `Game.LoadGameFromMainMenu`. The bounded repair now uses that path and fails early when `LoadingProcess` becomes inactive before the registered callback; it passes 3 deterministic watchdog tests and 11 new exact assembly-backed contract checks.

The native bootstrap retry executed farther but failed at the identical `Player.PostLoad` invariant. A bounded read-only check of only the two qualified archives proved both player/party JSON pairs parse and both contain the main-character identity in party UniqueId fields; Working additionally contains an explicit `CraftMagicItems` serialized-owner marker. This supported a party-deserialization failure caused by package-only staging.

The schema-3 transaction now creates a disposable byte-verified clone of the exact live Mods context and overlays only KMC. The guarded `20260813T233100Z-fixture-intake-overlay-pilot` run loaded that context and KMC, verified the exact Working campaign and area, and resolved exactly one valid Medium rider with the larger active Mammoth, enabled stock movement agent, and exact `Spine` anchor. It passed 3/3 subscenarios and 13/13 assertions. The authoritative live Mods tree and both qualified saves were restored exactly; no process, lock, sentinel, deployed KMC directory, or other external residue remains. This is fixture-compatibility evidence, while the two KMC-only no-save smokes remain the production package's standalone-load evidence.
...
Remaining work is the guarded live qualification of actual mounting, movement, drift, turns, doorway control, selection, formation, lifecycle residue, save/load/area boundaries, visual classification, and the final B/C/D architecture decision. Pair availability itself is PASS.
...
The invariant-correct larger Mammoth and exact live `Spine` anchor are verified, but no mount/movement screenshot or rider-pose evidence has yet been captured. No visual classification is claimed.

### Runtime safety state

Status: PASS

The harness, WhatIf, two final fresh-process smokes, guarded failure recoveries, schema-3 overlay pilot, and offline Working-slot recovery suite are qualified. Live Mods currently match the schema-3 transaction baseline/current inventory of 348 files/44 directories/69,797,076 bytes with digest `c292c5c62a232a0ad7b32ed489139a8d135caa18e38151677f619bf7555c70cb`; save metadata matches `58a54974423316041e29d32b1093c5d6775dbd67fe2b37ffaea3c9ae27de19be`. No KMC sentinel or lock and no Kingmaker/Wrath/UMM process remain. Older journal digests used different manifest algorithms and are not directly comparable.
