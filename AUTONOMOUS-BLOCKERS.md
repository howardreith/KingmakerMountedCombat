...
Stationary mounted rider melee in real time remains qualified twice. No critical blocker is active. TB probes A-F are immutable uncredited failures. Probe F identifies exact installed `UnitAttack.OnTick` as the caller that replaces an already-acted single attack's native `Success` with `Interrupt` when its one-second turn-based next-attack schedule finds no planned attack before animation completion. The narrow pair-child-only repair preserves that exact Success for ordinary base-command animation completion; it does not accept Interrupt or affect real time, pre-action, failed, missing-rule, incomplete, or multi-attack states. Complete offline gates pass. Exact next action is commit/publication/package/WhatIf followed by two fresh same-package TB PASS processes with independent restoration audits. Do not infer TB before both passes, or infer miss, invalid-target, target-death, non-mounted, or Mammoth-attack qualification; do not weaken gates, launch Wrath, touch another project, create a public release, or merge `main`.
