...
IN PROGRESS. No critical hard stop is proven. Isolation-C native/outer PASS, with exact human intake restored. P01-save-A made an actual native Manual write but failed metadata validation because global JSON defaults erased its fields. Preview.4 fixes that defect with a reproduction regression; native retry, cold round trip and P02-P08 remain IN PROGRESS/TODO. Actual human intake was restored. No critical native invariant is proven to block progress. See [the single report](docs/CHUNK5-PERSISTENCE.md). Human preview.54 is the current intake, not historical preview.37/preview.13. Isolation-B failed before fixture load; its owned process exited normally and exact human installation/data/settings were restored. No game or lock remains. Normal sandbox setup fails; individually reviewed bounded commands work.
