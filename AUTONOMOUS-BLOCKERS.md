...
Second guarded diagnostic `20260816T034500Z-rider-hit-rt-diagB` from clean commit `b947b466e4bd9a379c12bd2f8e150e1863405169` truthfully failed before combat on exactly six provisioning gates. Exact local contracts prove the predicates were formulated against the wrong facades: hostility is group-owned, the public AI getter is intentionally true for non-controllable units, native loot is `HasLoot`, and native single attacks select the first `HasWeapon` limb. The offline-green repair preserves every safety requirement while using those exact semantics, adds one disposable collision-checked runtime group, and independently proves entity/group/faction teardown. Commit and guarded-publish it, package clean HEAD, pass full-pin WhatIf, and run one fresh stationary real-time probe. Do not relax a gate, launch Wrath, touch another project, create a public release, merge `main`, or implement an unauthorized feature.
