...
Stationary mounted rider melee in real time remains qualified twice. No critical blocker is active. TB probes A/B/C are immutable uncredited failures. Probe C proves every native pre-click gate, then exposes a duplicated production-only `IsActing` admission check. Production and diagnostics now share one pure exact-rider `Preparing`/`Acting` action-window policy, while post-dispatch/outcome still require `Acting`; all offline gates and independent restoration pass. Exact next action is clean commit/publication/package/WhatIf and a fresh TB probe. Do not infer TB, miss, invalid-target, target-death, non-mounted, or Mammoth-attack qualification; do not weaken gates, launch Wrath, touch another project, create a public release, or merge `main`.
