...
Probe P `20260816T083000Z-rider-hit-rt-probeP` from guarded-published `f57f963f3afef1f5dcbee8cce83d0a9622686319` truthfully fails `20/1` before click. Both exact group-memory directions are present, but the four relevant units/groups never enter combat and the rider never prepares. Exact installed `UnitMemoryController.Tick` wakes the observer immediately after committing a buffered memory entry; the direct deterministic lease omitted that stock side effect. The narrow worktree calls `Wake()` only for a sleeping lease endpoint, retains the exact two-entry/timestamp/removal scope, and adds target awake/fog/peaceful deadline evidence. It never edits faction, stealth, initiative, cooldown, combat membership, production attack behavior, pathing, or saves. Complete offline gates pass (`21/0`, `192/0`, `17/0`, `155/0`, assembly `178/0`). Commit and guarded-publish, package clean HEAD, pass full-pin WhatIf, then run fresh Probe Q and independently audit restoration before evidence inspection. Do not weaken product/native gates, launch Wrath, touch another project, create a public release, or merge `main`.
