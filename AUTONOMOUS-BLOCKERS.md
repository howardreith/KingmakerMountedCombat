...
Probe E `20260816T050500Z-rider-hit-rt-probeE` from guarded-published `42a7c2412644d759764d8a738596bca69f2e898d` passes target weapon present/natural/melee and every non-source safety gate, but truthfully fails before target publication because the exact native primary weapon is not the same object as the blueprint empty-hand fallback. It receives no qualification credit. The narrow repair preserves native `PrimaryHand`, positive attack count, natural/melee type, and zero body mutation while classifying the stock source exactly as either a primary-hand item or empty-hand fallback; fallback identity must match its blueprint pin. Complete offline gates pass (`21/0`, `179/0`, `17/0`, `152/0`, assembly `148/0`). Commit and guarded-publish, package clean HEAD, pass full-pin WhatIf, then run one fresh stationary real-time probe. Do not relax a gate, launch Wrath, touch another project, create a public release, merge `main`, or implement an unauthorized feature.
