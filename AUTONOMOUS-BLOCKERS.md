...
Probe H `20260816T054500Z-rider-hit-rt-probeH` from guarded-published `34f9ac5e582d46a83c12374cfcbf06e889bb9962` proves the exact target-only click repair: all readiness gates pass, the native/Harmony click returns true, and the rider wrapper is admitted. It truthfully fails `27/2` because the wrapper never starts or ticks, with zero child attack, rule, roll, damage, cost, movement, or repath; it receives no qualification credit. Exact native audit narrows the start path to command ownership/queue state and the unchanged hands/equipment/CanAct/cooldown/range gates. The diagnostic worktree now rejects native non-admission and emits every exact start gate on timeout. Complete offline gates pass (`21/0`, `183/0`, `17/0`, `152/0`, assembly `153/0`). Commit and guarded-publish, package clean HEAD, pass full-pin WhatIf, run fresh Probe I, and repair only the proven gate. Do not bypass native action economy, alter another target or save, launch Wrath, touch another project, create a public release, or merge `main`.
