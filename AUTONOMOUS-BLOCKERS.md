...
Probe K `20260816T070000Z-rider-hit-rt-probeK` from guarded-published `0124cb6d3289f2ffce287c0c24bf3d100bd0b2f9` truthfully fails `39/7` after proving exact native memory entry, stock combat preparation, every dispatch gate, accepted click, and clean memory removal. Exact Mammoth distance `2.25020623` is within radius `2.37020588`, but stock child admission measures the offset rider executor and the wrapper exhausts four no-op repaths. There is no qualification credit. The narrow worktree keeps Mammoth-origin range and path authority exclusive, permits a measured rider-executor bridge only after that gate passes, adds `0.001` admission epsilon, caps the bridge at `0.75`, reapplies it through the exact child-only postfix, returns zero admission outside pair range, and retains stock Mammoth attack semantics. Complete offline gates pass (`21/0`, `189/0`, `17/0`, `154/0`, assembly `166/0`). Commit and guarded-publish, package clean HEAD, pass full-pin WhatIf, then run fresh Probe L and independently audit restoration before evidence inspection. Do not weaken product/native gates, launch Wrath, touch another project, create a public release, or merge `main`.
