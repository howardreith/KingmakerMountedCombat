...
Stationary mounted rider melee hit is qualified twice in both real-time and turn-based modes. No critical blocker is active. The first deterministic miss attempt is preserved uncredited `FAIL` `46/1`: mechanics and restoration pass, while the old diagnostic incorrectly expected literal `AttackResult.Miss` instead of observing authoritative `RuleAttackRoll.IsHit=false` plus exact reason `ArmorAC`. Exact installed getter tokens are now bound through backward-compatible schema v6/v7 and complete offline gates pass. Exact next action is commit/publication/package/WhatIf and fresh same-package miss A/B with independent audits. Invalid-target, target-death, cleanup, and non-mounted-control remain open. Do not infer qualification, weaken gates, launch Wrath, touch another project, create a public release, or merge `main`.
