# Active Chunk 4 - candidate37, 2026-09-20 UTC

IN PROGRESS. Native36 BR (`20260920-chunk4-BR`) is50/2 native/outer FAIL: four direct Charge controls pass, but the new Mount delivery hook rejects its command-lifecycle precondition before creating a Charge queue. Its original Mount still succeeds. All69 transactions independently restored the actual human `0.1.0-paired-preview.37` and protected data; no game/lock remains.

Candidate `0.1.0-chunk4-preview.37` changes only that diagnostic contract: native non-engaging casts may finish their command before custom delivery. It records the callback state before any assertion, traces the preceding Mount command, and binds pending delivery to the exact live ability execution context, caster and target. Active shells or genuinely completed Success shells are accepted; ended/mismatched execution, combat-before-Mount, resource mutation and missing real native work still fail. Build/source22/0 PASS; full37 checks PASS (exit0), Charge168/0 and Kingmaker569/0. Then commit/private package/guarded publish/fresh suite37 and focused Charge TB first (unique BS); RT and remaining exact-final gates follow. [Report](docs/CHUNK4-PLAYABILITY.md), [resume](AUTONOMOUS-RESUME.md). No permanent deployment or Chunk5 implementation.
