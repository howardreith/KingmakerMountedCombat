...
The native bootstrap retry executed farther but failed at the identical `Player.PostLoad` invariant. A bounded read-only check of only the two qualified archives proved both player/party JSON pairs parse and both contain the main-character identity in party UniqueId fields; Working additionally contains an explicit `CraftMagicItems` serialized-owner marker. This supports a party-deserialization failure caused by package-only staging. A separately audited clone-live-plus-KMC transaction is now the bounded next experiment. Its result will be reported as a modded-fixture compatibility result; the existing KMC-only no-save smokes remain the evidence that the production package has no gameplay-mod runtime dependency.
