...
Stationary mounted rider melee in real time remains qualified twice. No critical blocker is active. TB probes A/B are immutable uncredited failures. Probe B proves exact mounted combat entry, roster, and native rider `StartTurn`, then exposes Kingmaker's directly-controllable `Preparing` state: it becomes `Acting` only after a command is queued. The repaired row admits only the exact rider in `Preparing`/`Acting`, queues only the normal click path, and requires exact post-dispatch/outcome `Acting`; all offline gates and independent restoration pass. Exact next action is clean commit/publication/package/WhatIf and a fresh TB probe. Do not infer TB, miss, invalid-target, target-death, non-mounted, or Mammoth-attack qualification; do not weaken gates, launch Wrath, touch another project, create a public release, or merge `main`.
