...
Branch `codex/mounted-combat-phase3f-playable-core`; pre-checkpoint HEAD `4cb95af42ddf6a5ccc0fd714105d5321959088ed`. Preview.4 fixes the first actual save defect: native global JSON defaults erased unannotated pair data. An independent serializer now bypasses those defaults; the regression reproduces the native failure. Build/source22/0, data28/0, native storage contracts23/0; earlier unchanged component404/0 and harness250/0 remain separate. DLL `1af56111f0b8afb408755fa92db7d8d091fd8cd977d3bfe1b0e70935138f1818`, MVID `31d085bc-87f9-4478-b736-ada505782428`.

P01-save-A FAIL0/1 reported assertions on preview.3/source4cb95af: actual native Manual archive committed, but its KMC member was only11 bytes (`{"$id":"1"}`). No cold load was attempted. PID2260 exited normally; actual human preview.54, saves/settings/cache/foreign Mods restored at18:59:42Z. Isolation-C remains PASS14/0 native. No game or lock remains. Historical preview.37/preview.13 backups are separate.

Next: commit/package preview.4 as `p01-json-isolation`, admit suite5, run `persistence-p01-save` / `20260920-chunk5-P01-save-B`. After PASS, load that archive by actual hash in a new process. P01-P08 are incomplete; P02 combat restoration is not implemented. Receipt `analysis-cache/chunk5-persistence/ACTIVE.json` records exact source/package/suite/next command. First resume command: `git status --short`, then read the receipt.

The unchanged WhatIf return precedes all root creation/copy/staging/launch; actual full proof on8612544 plus focused current guards apply. Paired=true; legacy authorities/overlay=false. No permanent installation, main merge or release. Guarded publication currently through8612544; individually reviewed commands work despite normal sandbox setup failure.
