...
Branch `codex/mounted-combat-phase3f-playable-core`; source HEAD `ac7f0dba34460bf5cab3989e4be80a1f73f9c28b`. Preview.7 binds metadata restoration to the actual selected load enumeration and newly deserialized Player object. Exact native LoadRoutine calls PlayerState.PostLoad before assigning GameId; the old early campaign check rejected real actors. The archive/header owns early restoration, then published campaign/area identity must match before presentation. New world binding rejects old/replaced/canceled callbacks. Build/source22/0, components408/0, native archive contracts23/0; unchanged data28/0/harness250/0 are prior evidence. DLL `1d080296d571c52d1332cfa6b46abe77e7067986572f75feab5692243ecdbbb8`, MVID `76bd6522-303a-46a6-9dbf-bbfc93dc5a42`.

P01-save-D PASS23/0 native and PASS outer on preview.6/sourcef81749d. Real Manual archive `84ffb91c85fe10b0ccacc92befe9a1e4fd2be88e639f197f79bf9bd678c83270`; same pair/controls/current debt, ordinary ground movement and native rider attack delivered. PID16336 exited normally; full actual intake restored19:21:29Z. Fresh-process P01-load-A on that same candidate FAIL1/1 before pair attachment because early actor binding was absent. PID15624 exited normally; full actual intake restored19:23:46Z. Source archive unchanged. Prior A/B/C save failures retained. No game/lock remains.

Preview.7 packaging FAIL: the DLL is 4,197,376 bytes, 3,072 bytes above the existing 4 MiB entry cap. The failed `p01-load-world` ZIP is frozen and unqualified; no native preview.7 run occurred. The owner approved raising only the DLL entry cap to 5 MiB. The ZIP cap, Info.json cap, entry/dependency allowlists, hashes and runtime safeguards remain unchanged. The full harness passes253/0, including exact DLL, Info and ZIP size-boundary regressions. Next freeze the unchanged DLL under p01-load-world-cap5 before the native retry.

Next: qualify a new package identifier (never overwrite `p01-load-world`), create suite8, then run `persistence-p01-load` / `20260920-chunk5-P01-load-B` using `20260920-chunk5-P01-save-D` and the exact archive hash above. This causal retry uses no runner pair/action state. P01-P08 remain incomplete; combat persistence not implemented. Receipt `analysis-cache/chunk5-persistence/ACTIVE.json` holds exact package/suite/next command; first resume command `git status --short`, then read it.

Actual human preview.54, protected saves/settings/caches/foreign Mods and separate preview.37/preview.13 backups retained. Paired=true; legacy authorities/overlay=false. No permanent install/main merge/release. Guarded remote currently6b92133; the approved-cap checkpoint will follow.
