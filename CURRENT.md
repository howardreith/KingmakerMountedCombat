...
First gate PASS in both arrangements on preview9 (I/J,96/0). AB27 native log proves condition command admission, mount actor-end and forced split; rider continuation then fails the shared attack assertion. The host's 300-second limit interrupted cleanup before detailed export. Working28 adds exact completion observations, aligns the allocation host with its existing660-second engine plus60-second cleanup allowance, and exports failed cleanup after a30-second wait. No combat assertion is weakened. All28 A-AB transactions restored actual preview13/current state; source throughaac8826 is published. P05/P06, paired Mammoth TB, final A05/A10 and remaining supported cases remain unqualified.
