...
IN PROGRESS. Active mission: save-scoped relationships and legitimate action/turn continuity across actual saves and cold loads. [Single report](docs/CHUNK5-PERSISTENCE.md). Chunk 4 native engineering is accepted; visual/HUD/physical-input acceptance remains TODO.

Branch `codex/mounted-combat-phase3f-playable-core`; current pre-checkpoint HEAD `47b16ae09ef87c6623ee4db5bb3147d284e22a64`, a legitimate descendant of reviewed docs `8a297fa019ff205f50fd7b911b6f6e03d46fbb1a` and native source `429377d707a9976be65639e0c27954d8b4ff3717`. Candidate `0.1.0-chunk5-preview.2`: source/components/harness/native storage contracts/profile guards PASS 22/402/250/19/8, each with zero failures. DLL SHA256 `b8b8b9ae4445e285cbab8f7c2de639f89cbbe1f17057787dbd6082c705deb251`, MVID `4ac98ac1-725a-4e2a-94df-6f3479d138a3`.

Isolation-B failed constructing a Harmony12 iterator patch before fixture loading. Replaced it with tested native ZipSaver methods scoped to the selected load. B's owned process exited normally; actual human preview.54, all saves, profile/cache, UMM parameters and PlayerPrefs are restored; recovery receipt `runtime-evidence/20260920-chunk5-isolation-B/profile-recovery.json` PASS. No game or lock remains. Preserve preview.37/preview.13 historical backups. Guarded publication currently through `8612544`.

Next command: `git status --short`; package the clean preview.2 checkpoint as `isolation-native-scope`, create fresh suite3, and run guarded `persistence-isolation` as `20260920-chunk5-isolation-C`. Reuse the completed full WhatIf proof for the unchanged preflight/WhatIf branches. Continue to actual native P01 saving/cold loading after isolation passes. No P01-P08 native pass yet; production persistence remains unimplemented. Paired=true, legacy authorities/overlay=false. No permanent installation, main merge or release. Reviewed bounded commands remain necessary because normal sandbox setup fails.
