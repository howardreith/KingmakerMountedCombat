...
Branch `codex/mounted-combat-phase3f-playable-core`; pre-checkpoint HEAD `1b8747e7a619588c44ec237a1137e28c95fd90f8`, preserving reviewed docs/source ancestors. Development `0.1.0-chunk5-preview.3` implements the first out-of-combat metadata snapshot and dedicated early actor/presentation restore, with actual save/cold-load scenario code. It is unqualified for persistence; P01-P08 remain TODO. Build/source22/0, components404/0, harness250/0, native archive contracts23/0, primitive data25/0, owned-copy guards8/0. DLL `d378f41eb3240feb54b8b7def9de71172c7390fe1a99c75b7c37b522b589b9d7`, MVID `0acc580d-5432-4acc-9613-874345ceef36`.

Isolation-C PASS14/0 native and PASS outer, exact source `1b8747e` / preview.2: native enumeration/load/stash routing, source bytes unchanged, normal exit, actual human preview.54 and saves/settings/cache/foreign Mods restored. No game or lock remains. Original A/B failures and B recovery remain intact. Historical preview.37/preview.13 backups are separate.

Next: commit/package preview.3 as `p01-native-save`, admit fresh suite4 and run `persistence-p01-save` as `20260920-chunk5-P01-save-A`. Then use the written archive's hash and source run ID for `persistence-p01-load` in a fresh owned process. The request contains archive identity only. Actual package/source/suite receipts go in `analysis-cache/chunk5-persistence/ACTIVE.json`. First resume command: `git status --short`, then read that receipt.

The unchanged WhatIf return still precedes all root creation/copy/staging/launch; actual full WhatIf proof on8612544 plus current owned guard tests apply. No broad historical rescan per experiment. Paired=true; legacy authorities/overlay=false. No permanent installation, main merge or release. Guarded publication currently through8612544; reviewed commands work despite normal sandbox setup failure.
