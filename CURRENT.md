...
Preview.7 packaging FAIL: the DLL is 4,197,376 bytes, 3,072 bytes above the existing 4 MiB entry cap. The failed `p01-load-world` ZIP is frozen and unqualified; no native preview.7 run occurred. A tested proposal raises only the DLL cap to 5 MiB, retaining the ZIP cap, Info.json cap, entry/dependency allowlists and runtime safeguards. Owner approval is pending because AGENTS.md explicitly forbids weakening thresholds merely to pass a gate. The repository guard is unchanged.

Next after that decision: qualify a new package identifier (never overwrite `p01-load-world`), create suite8, then run `persistence-p01-load` / `20260920-chunk5-P01-load-B` using `20260920-chunk5-P01-save-D` and the exact archive hash above. This causal retry uses no runner pair/action state. P01-P08 remain incomplete; combat persistence not implemented. Receipt `analysis-cache/chunk5-persistence/ACTIVE.json` holds exact package/suite/next command; first resume command `git status --short`, then read it.

Actual human preview.54, protected saves/settings/caches/foreign Mods and separate preview.37/preview.13 backups retained. Paired=true; legacy authorities/overlay=false. No permanent install/main merge/release. Guarded remote currently3825383.
